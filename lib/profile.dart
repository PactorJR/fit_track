...
class _ProfilePageState extends State<ProfilePage> {
  ...
  Future<void> _editProfile() async {
    final TextEditingController firstNameController = TextEditingController(text: _userData?.get('firstName') ?? '');
    final TextEditingController lastNameController = TextEditingController(text: _userData?.get('lastName') ?? '');
    final TextEditingController emailController = TextEditingController(text: _userData?.get('email') ?? '');
    final TextEditingController phoneController = TextEditingController(text: (_userData?.get('phone')?.toString() ?? ''));
    final TextEditingController ageController = TextEditingController(text: (_userData?.get('age')?.toString() ?? ''));
    final TextEditingController birthdayController = TextEditingController(text: _userData?.get('birthday') ?? '');
                        ...
                        return GestureDetector(
                          ...
                          child: Container(
                            ...
                            decoration: BoxDecoration(
                              color: _selectedIconIndex == index ? Colors.blue.withOpacity(0.2) : Colors.transparent,
                              border: Border.all(
                ...
                TextButton(
                  onPressed: () async {
                    ...
                    await FirebaseFirestore.instance.collection('users').doc(_user.uid).update({
                      ...
                      'profileIconIndex': _selectedIconIndex ?? 0, // Update icon index
                    });
            ...
            child: Center(
              child: Padding(
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    color: Colors.green.withOpacity(1), // Slightly transparent green background
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      ProfileItem(
                        icon: Icons.email,
                        label: 'E-mail: ${_userData?.get('email') ?? 'Not available'}',
                      ),
                      ProfileItem(
                        icon: Icons.phone,
                      ...
                      ProfileItem(
                        icon: Icons.person_outline,
                        label: 'User Type: ${_userData?.get('userType') ?? 'Not available'}',
                      ),
                    ],
                  ),
                ),
